import type { ClusteredEvent, PerspectiveScore, AnalyzedHeadline, AnalysisReport } from './types.js';
...
class ParallelAnalysisService {
  ...
  private recentTitles = new Map<string, number>();
  ...
  async analyzeClusters(clusters: ClusteredEvent[]): Promise<AnalysisReport> {
    ...
    for (const cluster of clusters) {
      ...
      perspectives.push(this.analyzeKeywords(titleLower));
      perspectives.push(this.analyzeSentiment(title));
      perspectives.push(this.analyzeEntities(title));
      perspectives.push(this.analyzeNovelty(title));
      ...
      const flagReason = flagged
        ? disagreement > 0.3
          ...
          : 'Non-keyword signals flag as important'
        : undefined;

      const analyzedHeadline: AnalyzedHeadline = {
        id: cluster.id,
        title,
      ...
      if (flagReason) analyzedHeadline.flagReason = flagReason;
      analyzed.push(analyzedHeadline);
    }
    ...
    const report: AnalysisReport = {
      ...
      analyzed,
      topByConsensus: analyzed.filter(a => a.confidence > 0.55).slice(0, 10),
  ...
  private analyzeKeywords(titleLower: string): PerspectiveScore {
    let score = 0;
  ...
  private analyzeSentiment(title: string): PerspectiveScore {
    const negativeWords = ['crisis', 'war', 'attack', 'death', 'killed', 'crash', 'fail', 'collapse', 'threat', 'disaster', 'catastrophe', 'emergency'];
    const positiveWords = ['peace', 'agreement', 'success', 'growth', 'breakthrough', 'recovery', 'victory', 'celebrate'];
    const titleLower = title.toLowerCase();
    ...
    if (negCount === 0 && posCount === 0) {
      return { name: 'sentiment', score: 0.3, confidence: 0.5, reasoning: 'neutral tone' };
    }

    const isNegative = negCount > posCount;
    const intensity = Math.max(negCount, posCount);
    const score = isNegative ? Math.min(1, 0.4 + intensity * 0.2) : Math.max(0, 0.3 - intensity * 0.1);

    return {
      name: 'sentiment',
      score,
      confidence: Math.min(0.95, 0.6 + intensity * 0.1),
      reasoning: isNegative ? `negative(${negCount} indicators)` : `positive(${posCount} indicators)`,
    };
  }

  private analyzeEntities(title: string): PerspectiveScore {
    const entities: { text: string; type: string }[] = [];
    const titleLower = title.toLowerCase();
    const countries = ['iran', 'russia', 'china', 'israel', 'ukraine', 'north korea', 'syria', 'gaza', 'lebanon', 'yemen', 'saudi arabia', 'turkey'];

    for (const country of countries) {
      if (titleLower.includes(country)) entities.push({ text: country, type: 'LOC' });
    }

    const leaders = ['putin', 'xi', 'netanyahu', 'zelensky', 'khamenei'];
    for (const leader of leaders) {
      if (titleLower.includes(leader)) entities.push({ text: leader, type: 'PER' });
    }

    if (entities.length === 0) {
      return { name: 'entities', score: 0.2, confidence: 0.5, reasoning: 'no significant entities detected' };
    }

    const geopoliticalLocations = entities.filter(e =>
    ...
    return { name: 'entities', score: Math.min(1, score), confidence: 0.7, reasoning: reasons.join(' + ') };
  }

  private analyzeNovelty(title: string): PerspectiveScore {
    let maxSimilarity = 0;
    for (const [recentTitle] of this.recentTitles) {
      ...
      maxSimilarity = Math.max(maxSimilarity, intersection.length / Math.max(titleWords.size, recentWords.size));
    }

    this.recentTitles.set(title, Date.now());
    if (this.recentTitles.size > 100) {
      const firstKey = this.recentTitles.keys().next().value;
      if (firstKey) this.recentTitles.delete(firstKey);
    }

    const noveltyScore = 1 - maxSimilarity;
    return {
      name: 'novelty',
      score: Math.min(1, noveltyScore * 0.7 + (noveltyScore > 0.5 ? 0.3 : 0)),
      confidence: 0.6,
      reasoning: maxSimilarity > 0.7 ? `similar to recent story (${(maxSimilarity * 100).toFixed(0)}% overlap)` : `novel content (${(noveltyScore * 100).toFixed(0)}% unique)`,
    };
  }

  private analyzeVelocity(cluster: ClusteredEvent): PerspectiveScore {
    const hourAgo = Date.now() - 60 * 60 * 1000;
    const recentCount = cluster.allItems.filter(i => i.pubDate.getTime() > hourAgo).length;
    let score = 0.2;
    ...
    return { name: 'velocity', score, confidence: 0.8, reasoning };
  }

  private analyzeSources(cluster: ClusteredEvent): PerspectiveScore {
    const sources = cluster.sourceCount;
    if (sources >= 5) return { name: 'sources', score: 0.9, confidence: 0.9, reasoning: `${sources} sources - highly confirmed` };
    if (sources >= 3) return { name: 'sources', score: 0.7, confidence: 0.9, reasoning: `${sources} sources - confirmed` };
    if (sources >= 2) return { name: 'sources', score: 0.5, confidence: 0.9, reasoning: `${sources} sources - multi-source` };
    return { name: 'sources', score: 0.2, confidence: 0.9, reasoning: 'single source' };
  }

  private aggregateScores(perspectives: PerspectiveScore[]): { finalScore: number; confidence: number; disagreement: number } {
    let weightedSum = 0;
    let totalWeight = 0;
    let confidenceSum = 0;
    ...
    return {
      ...
      confidence: (confidenceSum / perspectives.length) * (1 - disagreement * 0.5),
      disagreement,
    };
  }

  private isLowKeywordScore(perspectives: PerspectiveScore[]): boolean {
    return (perspectives.find(p => p.name === 'keywords')?.score ?? 0) < 0.3;
  }
